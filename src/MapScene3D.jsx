...
export default function MapScene3D({ map, routePoints = [] }) {
  return <Canvas shadows orthographic camera={{ position: [10, 13, 15], zoom: 50 }} style={{ width: '100%', height: '100%' }}>
    ...
    <Walkways map={map}/><RouteOverlay points={routePoints}/>{(map.spaces || []).map(space => <SpaceMesh key={space.id} space={space}/>)}
